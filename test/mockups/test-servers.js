...
export async function startServers(options={}) {
  ...
  const CONTRACT_ABI_VERSION = '1.0';
  ...
  blockchainProvider = new blockchainProviders.EVMProvider(CONTRACT_ABI_VERSION, CHAIN_ID, new ethers.JsonRpcProvider(BLOCKCHAIN_SERVER_URL), options.protocol === 'ws' ? BUBBLE_WS_SERVER_URL : BUBBLE_SERVER_URL);
